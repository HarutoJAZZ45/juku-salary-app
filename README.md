...
- カレンダーのナビゲーションバーには **「今日に戻る」ボタン** が実装されており、今月以外の月を表示しているときのみ表示される。クリックで `currentViewDate` を `new Date()` にリセットします。
...
### 6.4 画面レイアウト（現行構成）
UIの縦方向のレイアウト順序は以下の通りです。

1. **ヘッダー** （ロゴ・各種ボタン群）
2. **SummaryCard** （当月給与見込み額・バッジ） ← 授業給/事務給ボックスは廃止
3. **カレンダーナビゲーション** （月移動矢印・今日に戻るボタン・複数選択ボタン）
4. **CalendarGrid** （月間カレンダー本体）
5. **TaxMonitor** （扶養控除管理ゲージ） ← カレンダー下に配置
...
- `deleteEntry(date)`: エントリを削除し、LocalStorage と Firestore の両方を即時更新します。
  - **重要**: Firestore への書き込みは `merge: true` を**使わず**、`{ entries: rest, config: settings }` を丸ごと上書き (`setDoc` without merge) します。これは、Firestore の `merge: true` がネストされた Map（`entries` 内の各日付キー）を再帰的にマージする仕様のため、削除したキーが Firestore に残存してしまうバグを防ぐためです。
...
### 7.3 RankingModal: 月表示ロジック
ランキングモーダルに表示する月の選択肢は、締め日（`closingDay`）ではなく**今日が何月か**のみを基準に算出されます。
- デフォルト選択（index 0）: **当月**（例：4月中なら `2026-04`）
- index 1: 前月
- index 2: 翌月

以前の実装は「締め日を過ぎていたら翌月をデフォルト表示」するロジックが存在しており、4月17日（締め日15日超過）に5月が先頭になるバグが発生していました。現在は締め日に依存しない実装に修正されています。
...
| カテゴリ | 内容 |
|---|---|
| **バグ修正** | ランキングの月選択: 4月中に5月がデフォルト表示されていた問題を修正。当月を先頭に前後1ヶ月を表示するよう変更 |
| **バグ修正** | エントリ削除時に Firestore の旧データが残存する問題を修正。Firestore の `merge:true` によるネスト Map 再帰マージ問題を回避するため、削除時は `config` と `entries` を一括上書き |
| **新機能** | カレンダーナビゲーションに「今日に戻る」ボタンを追加。今月以外を表示している場合のみ表示 |
| **UI改善** | `SummaryCard` から授業給・事務給の内訳ボックスを削除し、カード表示をシンプル化 |
| **UI改善** | `TaxMonitor`（扶養管理ゲージ）をカレンダー上部から下部に移動し、カレンダーの視認性を向上 |

---
*Document Version: 2.1.0 | Last Updated: 2026-04-17*
